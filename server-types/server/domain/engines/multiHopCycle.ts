...
export interface MultiHopCycle {
  path:                 string[];
  hops:                 number;
  totalLogWeight:       number;
  compoundedMultiplier: number;
  compoundedNetPct:     number;
}

export interface MultiHopDetectionResultFound {
  hasArbitrage: true;
  cycle:        MultiHopCycle;
}

export interface MultiHopDetectionResultNotFound {
  hasArbitrage: false;
  cycle:        null;
}

export type MultiHopDetectionResult = MultiHopDetectionResultFound | MultiHopDetectionResultNotFound;

export function isMultiHopCycle(obj: unknown): obj is MultiHopCycle {
...
export function isMultiHopDetectionResult(obj: unknown): obj is MultiHopDetectionResult {
  if (typeof obj !== 'object' || obj === null) return false;
  const r = obj as Record<string, unknown>;
  if (typeof r.hasArbitrage !== 'boolean') return false;
  if (r.hasArbitrage === false) return r.cycle === null;

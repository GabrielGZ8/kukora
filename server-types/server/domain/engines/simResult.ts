...
export interface SimResultExecution {
  ts:         string;
  pair:       string;
  netProfit:  number;
  score?:     number;
...
export interface SimResultEquityPoint {
  ts:     string;
  equity: number;
}

export interface SimResult {
  executions:     SimResultExecution[];
  equityCurve:    SimResultEquityPoint[];
  totalNetProfit: number;
  params:         Record<string, unknown>;
  /** Additional fields (sharpeRatio, pairStats, tradesExecuted, ...) are
   *  produced by some producers (e.g. simulateRun()) but are NOT part of
   *  this minimal contract — consumers that need them read them directly,
   *  isSimResult() does not require them. */
  [key: string]: unknown;
}
 ...
 */
export function isSimResult(obj: unknown): obj is SimResult {

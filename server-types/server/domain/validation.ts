...
export interface AlertCreateValue {
  coinId: string;
  coinName: string;
...
export interface AlertUpdateValue {
  condition?: 'above' | 'below';
  price?: number;
  triggered?: boolean;
}

export interface WatchlistSaveValue {
  coins: string[];
}

export interface PortfolioCreateValue {
  coinId: string;
  coinName: string;
  symbol: string;
  quantity: number;
...
function cleanString(v: unknown, maxLen: number = MAX_STRING_LEN): string | null {
  if (typeof v !== 'string') return null;
  const trimmed = v.trim();
  if (!trimmed.length || trimmed.length > maxLen) return null;
  return trimmed;
}

export function validateAlertCreate(body: any): ValidationResult<AlertCreateValue> {
  ...
  if (!coinId) return { valid: false, error: 'coinId is required (1-100 chars)' };

  const coinName = cleanString(body.coinName, 100);
  if (!coinName) return { valid: false, error: 'coinName is required (1-100 chars)' };
...
export function validateAlertUpdate(body: any): ValidationResult<AlertUpdateValue> {
  if (!body || typeof body !== 'object') return { valid: false, error: 'Request body must be an object' };
  const value: AlertUpdateValue = {};
...
export function validateWatchlistSave(body: any): ValidationResult<WatchlistSaveValue> {
  if (!body || typeof body !== 'object') return { valid: false, error: 'Request body must be an object' };
  if (!Array.isArray(body.coins)) return { valid: false, error: 'coins must be an array' };
  if (body.coins.length > MAX_COINS) return { valid: false, error: `coins cannot exceed ${MAX_COINS} entries` };

  const coins: string[] = [];
  for (const c of body.coins) {
    const clean = cleanString(c, 100);
    if (!clean) return { valid: false, error: 'Each coin id must be a non-empty string (max 100 chars)' };
    coins.push(clean);
  }
...
export function validatePortfolioCreate(body: any): ValidationResult<PortfolioCreateValue> {
  if (!body || typeof body !== 'object') return { valid: false, error: 'Request body must be an object' };
  ...
  if (!symbol) return { valid: false, error: 'symbol is required (1-20 chars)' };
  ...
  const value: PortfolioCreateValue = { coinId, coinName, symbol, quantity: body.quantity, entryPrice: body.entryPrice };
  if (image) value.image = image;
  if (body.entryDate !== undefined) {
    const d = new Date(body.entryDate);
    if (Number.isNaN(d.getTime())) return { valid: false, error: 'entryDate must be a valid date' };
    value.entryDate = d;
  }
...
export function parsePagination(query: any): PaginationResult {
...
// ─── Arbitrage engine config validation ────────────────────────────────────
// Validates POST /api/arbitrage/config body to prevent disabling risk controls.
// Guards against a client sending { maxDailyLossUSD: 0, maxDrawdownPct: 0 }
// which would neutralize safety circuit breakers silently.
...
const RISK_SAFETY_FLOORS: Record<string, RiskFloor> = {
...
export function validateArbitrageConfig(body: any): ValidationResult<any> {
  ...
  if (body.scoringWeights !== undefined) {
    ...
    const total = Object.values(body.scoringWeights as Record<string, unknown>)

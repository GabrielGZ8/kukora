...
const DEFAULTS = {
  ...
  crashPct:   -8,     // caída en 1 vela = crash
  spikePct:    8,     // subida en 1 vela = spike
  volMult:     2.5,   // veces la volatilidad histórica = inusual
};

// z-score del último retorno vs distribución histórica
...
const detectAnomalies = (prices, opts = {}) => {
  ...
  if (lastRet <= cfg.crashPct) {
    ...
    details.push({ type: 'crash', label: 'Caída brusca', value: `${lastRet.toFixed(2)}%`, score });
    totalScore += score;
  }
  ...
  if (Math.abs(z) >= cfg.zThreshold) {
    ...
    details.push({ type: 'zscore', label: `Retorno estadísticamente anómalo (${dir})`, value: `z=${z.toFixed(2)}`, score });
    totalScore += score;
  }
  ...
  if (histVol > 0 && recentVol > histVol * cfg.volMult) {
    ...
    details.push({ type: 'volatility', label: 'Volatilidad inusual', value: `${recentVol.toFixed(2)}x histórico`, score });
    totalScore += score;
  }

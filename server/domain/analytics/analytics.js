// ─── analytics.js — reusable quantitative analytics layer ──────────────
// Todas las funciones: (prices: number[]) → scalar o array
// No external dependencies. Pragmatic, debuggable, fast to modify.
...
const trendDetection = (prices, shortPeriod = 10, longPeriod = 30) => {
  ...
  const label = trend === 'bullish' ? '▲ Bullish' : trend === 'bearish' ? '▼ Bearish' : '→ Sideways';
...
// ── Beta vs benchmark (simultaneous returns) ──────────────────────────────
...
const marketRegime = (prices) => {
  ...
  if (highVol && dd > 15)       { regime = 'volatile';      label = '~ Volatile market';       color = 'red'; }
  else if (trend.trend === 'bullish') { regime = 'trending_up';   label = '▲ Uptrend';    color = 'green'; }
  else if (trend.trend === 'bearish') { regime = 'trending_down'; label = '▼ Downtrend';    color = 'red'; }

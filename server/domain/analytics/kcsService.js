...
// Proprietary metric combining 7 market dimensions
// Score 0-100, bias (BULLISH/BEARISH/NEUTRAL), market state

const { percentageChange, stdDev, clean, last } = require('./analytics');
...
const computeKCS = (pricesArr, volumeArr = null, btcDominance = null, fearGreed = null) => {
  ...
  // ── Component 2: VOLATILITY (15%) — lower vol = stronger signal ──────────
  ...
  // Optimal RSI: 45-65 (neither overbought nor oversold)
  ...
  if (score >= 72) {
    ...
    color = '#00E5A0';
    description = 'Favorable conditions for long exposure. Multiple signals converging positively.';
  } else if (score >= 55) {
    ...
    color = '#FFD166';
    description = 'Balanced signals. Market in transition. Wait for directional catalyst.';
  } else if (score >= 30) {
    bias = 'BEARISH'; state = 'CAUTIOUSLY BEARISH';
    color = '#FF8C42';
    description = 'Gradual deterioration of conditions. Reduce exposure, manage risk actively.';
  } else {
    bias = 'BEARISH'; state = 'RISK OFF';
    color = '#FF4D6A';
    description = 'Adverse conditions. Multiple bearish signals. Preserve capital as priority.';
  }

...
const detectSignals = (prices, indicators) => {
  ...
  if (rsiVal > 70) signals.push({ type: 'bearish', label: 'Sobrecomprado', desc: `RSI ${rsiVal.toFixed(1)} > 70`, ts });
  else if (rsiVal < 30) signals.push({ type: 'bullish', label: 'Sobrevendido', desc: `RSI ${rsiVal.toFixed(1)} < 30`, ts });
  ...
  const n = hist.length;
  if (n >= 2) {
    if (hist[n - 2] < 0 && hist[n - 1] > 0) signals.push({ type: 'bullish', label: 'Bullish MACD signal', desc: 'MACD crosses signal upward', ts });
    else if (hist[n - 2] > 0 && hist[n - 1] < 0) signals.push({ type: 'bearish', label: 'Bearish MACD signal', desc: 'MACD crosses signal downward', ts });
  }

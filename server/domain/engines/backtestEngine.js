...
const runBacktest = (prices, strategyKey) => {
  ...
  const runResult = { strategy: result, benchmark: bh };

  // Chequeo de contrato NO bloqueante (auditoría de comité 2026-07-08,
  // hoja de ruta #1): backtestEngine.js es el único productor de este
  // shape, consumido por datasetService.js y crypto.routes.js. Un drift
  // aquí no debe romper la respuesta, solo señalarlo para diagnóstico.
  if (!isBacktestRunResult(runResult)) {
    observability.emit('ENGINE', 'contract.backtest_run_result_shape_invalid', { strategyKey });
  }
...
const runAllStrategies = (prices) => {
  ...
  const invalidKeys = Object.entries(results)
    .filter(([, r]) => !isBacktestRunResult({ strategy: r, benchmark: r }))
    .map(([key]) => key);
  if (invalidKeys.length > 0) {
    observability.emit('ENGINE', 'contract.backtest_run_result_shape_invalid', { keys: invalidKeys });
  }

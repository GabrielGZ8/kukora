/**
 * exchangeIntelligence.js — Kukora Platform Module
 *
 * Consolidates:
...
//
// Computes rolling volatility using log-returns (quant/HFT standard).
// Tres ventanas: short (micro), mid (trading), long (regime).
//
// Thresholds BTC/USD spot:
//   STABLE    score <30   — trades normales
//   CAUTION   score 30-64 — reduce size
//   HIGH RISK score >=65  — block execution
...
/**
 * getBtcPriceSeries - ADR-019 Sec4 read access into the rolling BTC price
 * buffer _updateVolatility() already maintains, so marketRegimeEngine can
 * reuse the existing accumulator instead of a second one. Read-only, does
 * not mutate anything.
 */
function getBtcPriceSeries() {
  return _priceBuffer.map(p => p.price);
}
...
// ─── Execution Penalty (ADR-019 §3) ───────────────────────────────────────
/**
 * getExecutionPenalty - ADR-019 §3: a real execution-outcome reliability
 * penalty, derived from this module's own success/failure history per
 * exchange (as opposed to exchangeReliabilityDynamic.getDynamicPenalty(),
 * which is feed-health based: WS uptime/staleness/latency/drops). The two
 * penalties measure genuinely different failure modes (did the feed look
 * healthy vs did trades on this exchange actually succeed), so they are
 * combined via Math.max (worst-of) at the call site
 * (opportunityDetection.js's reliabilityPenalty), never summed — see
 * ADR-019 Part A §1 for the full rationale.
 *
 * Returns 0 (no penalty) when:
 *   - executionPenaltyEnabled is false
 *   - the exchange has fewer than minExecutionSamples total trades
 *     (successCount + failureCount) — insufficient sample, don't punish
 *     an exchange we simply haven't traded on enough yet.
 *
 * Otherwise returns a 0-100 penalty scaled from the failure rate: a 0%
 * failure rate is 0 penalty, a 100% failure rate is 100 penalty (i.e. the
 * penalty *is* the failure rate, not a different curve — kept simple and
 * auditable per ADR-019's "no magic constants" guidance).
 */
function getExecutionPenalty(exchange) {
  const liveConfig = require('./liveConfig');
  if (!liveConfig.get('executionPenaltyEnabled')) return 0;

  const s = _exStats[exchange];
  if (!s) return 0;

  const totalSamples = s.successCount + s.failureCount;
  const minSamples = liveConfig.get('minExecutionSamples');
  if (totalSamples < minSamples) return 0;

  const failureRate = s.failureCount / totalSamples;
